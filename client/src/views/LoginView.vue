  ...
  <div class="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-indigo-900 via-slate-900 to-slate-950 p-4 relative overflow-hidden">
    ...
    <div class="w-full max-w-md bg-white/10 backdrop-blur-xl border border-white/20 p-8 rounded-2xl shadow-2xl relative z-10 text-white">
      <div class="flex flex-col items-center mb-8">
        ...
        <h1 class="text-2xl font-bold tracking-tight">OpenMindMap</h1>
        <p class="text-sm text-slate-300 mt-1">自建云端思维导图与知识库平台</p>
      </div>
      ...
      <form @submit.prevent="handleLogin" class="space-y-5">
        <div>
          <label class="block text-xs font-medium text-slate-300 uppercase tracking-wider mb-2">用户名</label>
          <input
            v-model="username"
            type="text"
            required
        ...
        <div>
          <label class="block text-xs font-medium text-slate-300 uppercase tracking-wider mb-2">密码</label>
          <input
            v-model="password"
            type="password"
            required
      ...
      <div class="mt-6 text-center text-sm text-slate-300">
        还没有账号？
        <router-link to="/register" class="text-indigo-400 hover:text-indigo-300 font-medium ml-1 underline underline-offset-4">
          免费注册
        </router-link>
      </div>
    </div>
...
const handleLogin = async () => {
  ...
  try {
    ...
    errorMessage.value = err.response?.data?.error || '登录失败，请检查网络或用户名密码';
  } finally {
    loading.value = false;
  }
};
